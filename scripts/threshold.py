...
class ThresholdSlider(QWidget):
    def __init__(self, update_callback, store_value_callback, dogs, mins, min_val=25, max_val=150):
        super().__init__()
    ...
    def set_status(self, value, dog, mind):
    ...
    def get_status(self):
        ...
        return value, dog_preset, dis_preset

    def store_value(self):
        self.store_value_callback()

    def on_update(self, _):
        v, dog, mind = self.get_status()
        self.update_callback(v, dog, mind)
...
class GuiProcess:
    ...
    def __init__(self, directory, jsonpath):

        self.directory = directory
        self.current_frame = 0
        self.image_data, self.peak_data_list, self.stems = load_tiffs_and_peaks(directory)
    ...
    def store_preset(self):
        value, dog, mind = self.slider.get_status()
        stem = self.stems[self.current_frame]
        self.store_value_callback(stem, {"value": value, "dog": dog, "mind": mind})
    ...
    def update_frame(self):
        ...
        if self.get_preset_callback(self.stems[self.current_frame]) is not None:
            ...
            mind = preset["mind"]
            value = preset["value"]
            self.slider.set_status(value, dog, mind)

        else:
            value, dog, mind = self.slider.get_status()
            self.update_peaks(value, dog, mind)
    ...
    def update_peaks(self, value, dog_preset, min_distance_preset):
...
def main():
    ...
    gui_process = GuiProcess(directory, jsonpath)
    gui_process.run()
