...
def process_file(infile, output_dir):
    ...
    # convert to 16-bit
    axis_normalizer_max = rolling_avg(np.quantile(raw, 0.99, axis=(0, 1)))
    axis_normalizer_max[:START] = axis_normalizer_max[START]
    axis_normalizer_max[END:] = axis_normalizer_max[END]
    axis_normalizer_min = rolling_avg(np.quantile(raw, 0.01, axis=(0, 1)))
    axis_normalizer_min[:START] = axis_normalizer_min[START]
    axis_normalizer_min[END:] = axis_normalizer_min[END]
    data = (raw - axis_normalizer_min) / (axis_normalizer_max - axis_normalizer_min)
